...
class StimMaker:
    ...
    def _return_rect_for_stim(self, DISPLAYSURF, rect_to_draw, color):
        """this function must return a pygame.rect object
        which the StimMaker.make_stim method will `blit` on
        the stimulus."""
        rect = pygame.draw.rect(DISPLAYSURF, colors_dict[color], rect_to_draw)
        return rect
    ...
    def make_stim(self):
        ...
        # set up window
        DISPLAYSURF = pygame.display.set_mode(self.window_size, 0, 32)
        ...
        for item in range(self.set_size):
            ...
            if item in target_inds:
                curr_rect = self._return_rect_for_stim(DISPLAYSURF,
                                                       rect_to_draw,
                                                       self.target_color)
            else:
                curr_rect = self._return_rect_for_stim(DISPLAYSURF,
...
class RectangleStimMaker(StimMaker):
    def _return_rect_for_stim(self, DISPLAYSURF, rect_to_draw, color):
        rect = pygame.draw.rect(DISPLAYSURF, colors_dict[color], rect_to_draw)
        return rect
